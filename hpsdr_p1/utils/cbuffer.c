...
int cbuf_offer(cbuf_t *me, void *data, const int size) {
    /* prevent buffer from getting completely full or over commited */
    if (cbuf_unusedspace(me) <= size) return 0;

    int written = cbuf_unusedspace(me);
    written = size < written ? size : written;
    memcpy(me->data + me->tail, data, written);
    me->tail += written;
    if (me->size < me->tail) me->tail %= me->size;
    return written;
}

void* cbuf_peek(const cbuf_t *me) {
    if (cbuf_is_empty(me)) return NULL;

    return me->data + me->head;
}

void* cbuf_poll(cbuf_t *me, const unsigned int size) {
    if (cbuf_is_empty(me)) return NULL;

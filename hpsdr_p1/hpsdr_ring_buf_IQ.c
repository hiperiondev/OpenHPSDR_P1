...
cbuf_handle_t circular_buf_init(iq_t *buffer, size_t size) {
...
/// modified by get.
void circular_buf_put(cbuf_handle_t me, iq_t data) {
    assert(me && me->buffer);
...
int circular_buf_try_put(cbuf_handle_t me, iq_t data) {
    assert(me);
    assert(me->buffer);
...
int circular_buf_get(cbuf_handle_t me, iq_t *data) {
    assert(me);
    assert(data);
    assert(me->buffer);
...
int circular_buf_peek(cbuf_handle_t me, iq_t *data, unsigned int look_ahead_counter) {
    int r = -1;
    size_t pos;
